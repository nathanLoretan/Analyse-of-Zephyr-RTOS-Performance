...
static int spim_nrf52_init(struct device *dev)
{
...
#if CONFIG_SPI_0
	if(!strcmp(dev->config->name, CONFIG_SPI_0_NAME))
	{
		port = CONFIG_SPIM0_NRF52_GPIO_PORT;
		if(CONFIG_SPIM0_NRF52_GPIO_PORT == 0) {
			data->gpio_port = device_get_binding(CONFIG_GPIO_NRF5_P0_DEV_NAME);
		}
		else if (CONFIG_SPIM0_NRF52_GPIO_PORT == 1) {
			data->gpio_port = device_get_binding(CONFIG_GPIO_NRF5_P1_DEV_NAME);
		}
	}

	#if CONFIG_SPI_1
		else if(!strcmp(dev->config->name, CONFIG_SPI_1_NAME))
		{
			port = CONFIG_SPIM1_NRF52_GPIO_PORT;
			if(CONFIG_SPIM1_NRF52_GPIO_PORT == 0) {
				data->gpio_port = device_get_binding(CONFIG_GPIO_NRF5_P0_DEV_NAME);
			}
			else if (CONFIG_SPIM1_NRF52_GPIO_PORT == 1) {
				data->gpio_port = device_get_binding(CONFIG_GPIO_NRF5_P1_DEV_NAME);
			}
		}
	#endif /* CONFIG_SPI_1 */
#else
	#if CONFIG_SPI_1
		if(!strcmp(dev->config->name, CONFIG_SPI_1_NAME))
		{
			port = CONFIG_SPIM1_NRF52_GPIO_PORT;
			if(CONFIG_SPIM1_NRF52_GPIO_PORT == 0) {
				data->gpio_port = device_get_binding(CONFIG_GPIO_NRF5_P0_DEV_NAME);
	...
	spim->PSEL.SCK 	= config->psel.sck | ((port << SPIM_PSEL_SCK_PORT_Pos) & SPIM_PSEL_SCK_PORT_Msk);
	spim->PSEL.MOSI = config->psel.mosi | ((port << SPIM_PSEL_MOSI_PORT_Pos) & SPIM_PSEL_MOSI_PORT_Msk);
	spim->PSEL.MISO = config->psel.miso | ((port << SPIM_PSEL_MISO_PORT_Pos) & SPIM_PSEL_MISO_PORT_Msk);
